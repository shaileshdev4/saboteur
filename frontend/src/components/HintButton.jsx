...
export default function HintButton({
  ...
  const isTerminal = hints.length > 0 && hints[hints.length - 1].is_terminal;
  const cumulative =
  ...
  const tierDots = (
    <div
      ...
      aria-label={`${hints.length} of 3 hints used`}
    >
      {[1, 2, 3].map((t) => (
        <span
          key={t}
    ...
    <div className="mt-2 pt-2 border-t border-warn/30 space-y-1.5">
      {hints.map((h) => (
        ...
        >
          <Chip
            variant="warn"
            className="inline-flex mr-2 text-[10px] py-0 px-1.5"
          >
            T{h.tier}
          </Chip>
          {h.message}
        ...
        <Card variant="default" className="p-2.5 sm:p-3 panel-hint">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            <span className="label-hint inline-flex items-center gap-1.5">
              <Lightbulb
                size={14}
                className="text-warn-foreground"
                aria-hidden
              />
              Hints
            </span>
            ...
            {isTerminal && (
              <span className="text-xs text-warn-foreground/80">
                All hints used
              </span>
            )}
            {cumulative > 0 && (
