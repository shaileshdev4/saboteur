...
1. **Multimodal LLM** (default if MULTIMODAL_API_KEY is set). We send the
   image base64 to a vision model and ask it to transcribe the math as
   plain text. Supports Anthropic Messages API (`/messages`) or any
   OpenAI-compatible API such as Groq/OpenAI (`/chat/completions`). Set
   `MULTIMODAL_API_STYLE=openai|anthropic` to override auto-detection.
...
def _multimodal_uses_openai_api() -> bool:
    """Groq/OpenAI use /chat/completions; Anthropic uses /messages."""
    style = os.environ.get("MULTIMODAL_API_STYLE", "").strip().lower()
    if style == "openai":
        return True
    if style == "anthropic":
        return False
    return "anthropic.com" not in MULTIMODAL_API_BASE.lower()


def _via_multimodal_llm(image_bytes: bytes, content_type: str,
                        hint: Optional[str]) -> dict:
    if _multimodal_uses_openai_api():
        return _via_multimodal_openai(image_bytes, content_type, hint)
    return _via_multimodal_anthropic(image_bytes, content_type, hint)


def _via_multimodal_openai(image_bytes: bytes, content_type: str,
                           hint: Optional[str]) -> dict:
    """OpenAI-compatible vision (Groq, OpenAI, etc.)."""
    b64 = base64.b64encode(image_bytes).decode("ascii")
    user_text = (f"Transcribe this. {hint or ''}".strip()
                 or "Transcribe this math work.")
    payload = {
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            r = client.post(f"{MULTIMODAL_API_BASE}/chat/completions",
...
def _via_multimodal_anthropic(image_bytes: bytes, content_type: str,
                              hint: Optional[str]) -> dict:
